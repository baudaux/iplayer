...
std::string CmdPlay::name = "play";

std::string & CmdPlay::getName() {

  return name;
}

bool CmdPlay::execute(const std::vector<std::string> & args, IPlayer & iplayer) {

  (void)args;
    
  iplayer.getPlaylist().play(iplayer);
  
  return true;
}

void CmdPlay::displayHelp() {

  std::cout << std::setw(MIN_WIDTH) << std::left << name << ": Start/resume playback." << std::endl;
}
