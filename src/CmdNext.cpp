...
std::string CmdNext::name = "next";

std::string & CmdNext::getName() {

  return name;
}

bool CmdNext::execute(const std::vector<std::string> & args, IPlayer & iplayer) {

  (void)args;

  iplayer.getPlaylist().next(iplayer);
  
  return true;
}

void CmdNext::displayHelp() {

  std::cout << std::setw(MIN_WIDTH) << std::left << name << ": Play the next track of the playlist." << std::endl;
}
